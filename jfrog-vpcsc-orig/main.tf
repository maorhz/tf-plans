...
resource "google_access_context_manager_access_policy" "access_policy" {
  parent = "organizations/323910511922" # CHANGE ORG ID
  title  = "POLICY_B"
}
...
resource "google_access_context_manager_service_perimeter" "jfrog_perimeter" {
  ...
  status {
    ...
    resources = ["projects/1022605794612"]
    ...
    depends_on = [
        google_access_context_manager_access_level.any_sa_except_one_from_ip,
        google_access_context_manager_access_level.specific_sa_from_ip
    ]
}
...
resource "google_access_context_manager_access_level" "any_sa_except_one_from_ip" {
  parent = "accessPolicies/${google_access_context_manager_access_policy.access_policy.name}"
  name   = "accessPolicies/${google_access_context_manager_access_policy.access_policy.name}/accessLevels/anySaExceptOneFromIp"
...
resource "google_access_context_manager_access_level" "specific_sa_from_ip" {
  parent = "accessPolicies/${google_access_context_manager_access_policy.access_policy.name}"
  name   = "accessPolicies/${google_access_context_manager_access_policy.access_policy.name}/accessLevels/specificSaFromIp"
